...
// When the user clicks anywhere outside the modal dialog, close it
...
function cellClicked(id) {
    ...
    const cell = parseInt(idName[idName.length - 1]);
...
function checkWin() {
    ...
    for (i = 0; i <= 2; i++) {
        ...
        if (col[0] > 0 && col[0] === col[1] && col[0] === col[2]) {
            ...
            // Give the winning row/column/diagonal a different bg-color
            tmpAr = myGrid.getColumnIndices(i);
            for (j = 0; j < tmpAr.length; j++) {
                str = "cell" + tmpAr[j];
                document.getElementById(str).classList.add("win-color");
            }
